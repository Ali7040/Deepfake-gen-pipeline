...
from deeptrace.audio import create_empty_audio_frame
from deeptrace.content_analyser import analyse_image
from deeptrace.filesystem import is_image
from deeptrace.processors.core import get_processors_modules
from deeptrace.temp_helper import clear_temp_directory, create_temp_directory, get_temp_file_path
from deeptrace.time_helper import calculate_end_time
from deeptrace.types import ErrorCode
from deeptrace.vision import conditional_merge_vision_mask, detect_image_resolution, extract_vision_mask, pack_resolution, read_static_image, read_static_images, restrict_image_resolution, scale_resolution, write_image
...
def setup() -> ErrorCode:
	"""
	Clear and create the temporary directory for processing.
	"""
...
def prepare_image() -> ErrorCode:
	"""
	Scale and restrict the target image resolution for processing.
	"""
...
def process_image() -> ErrorCode:
	...
	if target_vision_frame.shape[2] == 4:
		reference_vision_frame = target_vision_frame[:, :, :3]
	else:
	...
	target_vision_frame = target_vision_frame # type: ignore
...
def finalize_image(start_time : float) -> ErrorCode:
	"""
	Finalize the output image by scaling it back and saving it to the output path.
	"""
	output_image_resolution = scale_resolution(detect_image_resolution(state_manager.get_item('target_path')), state_manager.get_item('output_image_scale'))
