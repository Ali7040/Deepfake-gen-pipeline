...
from deeptrace import ffmpeg
from deeptrace import logger, process_manager, state_manager, translator, video_manager
from deeptrace.audio import create_empty_audio_frame, get_audio_frame, get_voice_frame
from deeptrace.common_helper import get_first
from deeptrace.content_analyser import analyse_video
from deeptrace.filesystem import filter_audio_paths, is_video
from deeptrace.processors.core import get_processors_modules
from deeptrace.temp_helper import clear_temp_directory, create_temp_directory, move_temp_file, resolve_temp_frame_paths
from deeptrace.time_helper import calculate_end_time
from deeptrace.types import ErrorCode, Fps, VisionFrame
from deeptrace.vision import conditional_merge_vision_mask, detect_video_resolution, extract_vision_mask, pack_resolution, read_static_image, read_static_images, read_static_video_frame, restrict_trim_frame, restrict_video_fps, restrict_video_resolution, scale_resolution, write_image
from deeptrace.workflows.core import is_process_stopping
		...
		with tqdm(total = len(temp_frame_paths), desc = translator.get('processing'), unit = 'frame', ascii = ' =', disable = state_manager.get_item('log_level') in [ 'warn', 'error' ]) as progress:
			...
			with ThreadPoolExecutor(max_workers = state_manager.get_item('execution_thread_count')) as executor:
				...
				for frame_number, temp_frame_path in enumerate(temp_frame_paths):
					future = executor.submit(process_temp_frame_partial, temp_frame_path = temp_frame_path, frame_number = frame_number)
...
def process_temp_frame(temp_frame_path : str, frame_number : int, reference_vision_frame : VisionFrame, source_vision_frames : List[VisionFrame], source_audio_path : Optional[str], temp_video_fps : Fps) -> bool:
