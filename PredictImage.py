...
print(f"Booting DeepTrace Engine on: {DEVICE}")

mtcnn = MTCNN(keep_all=True, device=DEVICE)

# The functional Swin Transformer replacement
MODEL_ID = "Purnachander-Konda/deepfake-detection-swin"

print(f"Downloading and Loading {MODEL_ID} from Hugging Face...")

# Download and load the pre-trained weights and configurations
    ...
    if hasattr(model.config, 'id2label'):
        ...
        for idx, label in id2label.items():
            if "fake" in label.lower():
                return probabilities[int(idx)].item()
                
    return probabilities[1].item() if len(probabilities) > 1 else probabilities[0].item()
...
def predict_face_deepfake(request):
    ...
    for i, box in enumerate(boxes):
        ...
        # 15% Margin to capture blending edges without excessive background noise
